...
def test_gallery_sync_orders_and_filters_images(monkeypatch) -> None:
...
def test_delete_gallery_removes_unindexed_owner_files(monkeypatch) -> None:
    root = Path.cwd() / "data" / f"test_gallery_cleanup_{uuid4().hex}"
    try:
        monkeypatch.setenv("JUSTRAYZIST_ROOT", str(root))
        settings = load_settings()
