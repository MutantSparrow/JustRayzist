...
def test_inference_upscale_persists_lineage(monkeypatch, temp_app_paths, make_app_settings) -> None:
    settings = make_app_settings(paths=temp_app_paths)
    service = InferenceService(settings=settings)
    saved_metadata: dict[str, object] = {}
    ...
    result = service.upscale(owner_id="example-client", filename="source.png", seed=999)
    ...
    assert metadata["source_width"] == 320
    assert metadata["source_height"] == 240
    assert metadata["upscale_engine"] == DEFAULT_UPSCALE_ENGINE_NAME
    assert metadata["execution_mode"] == DEFAULT_UPSCALE_ENGINE_NAME
    assert metadata["fidelity_seed_ms"] == 11
